...
class Test{
	int iv; //instance variable 객체변수
	// 이 변수는 초기화를 하지 않아도 된다 (기본값: default) 가지고 있다
	// why : int iv = 100; 초기화를 하지 않아도 사용가능하게 했을까
	// 객체마다 다른 값을 (아파트마다 다른 층수를) 갖게 하기 위해서
	
	void print() {
		int i = 100; //함수안에 있는 변수를 local variable 지역변수
					 //함수가 호출되면 그때 stack 메모리에 생성되고 함수가 종료되면 같이 소멸
					 //지역변수는 바드시 초기화를 해야한다 (암기)
	...
	void write() {
		...
		int i = 200; // 언제라도 가능함
		// int iv = 300; Error 는 아니지만 의미가 없음
	}
