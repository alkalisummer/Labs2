/*
 ...
 숫자 -> 정수(음의 정수, 0, 양의 정수) -> byte(-128~127)
     ...
     -> 실수(부동소수점)  -> float(4byte), double(8byte)
     						정밀도가 높다(표현범위가크다) **java 실수 연산의 기본타입 (double)
	 -> boolean(true, false) : 프로그램의 논리 제어
	 
 궁금해요 : 문자열의 표현 : 홍길동, 안녕하세요 어디에 담을까
 String는 클래스 : String s = new String("안녕하세요");
 앞 예제 : String s = "안녕하세요";
 String 대해서 별도의 언급이 있기 까지는 String도 int처럼 쓰세요
 ...
 */
// 최소한 설계도라고 할려면 >> 속성(상태정보)과 행위(기본)를 갖고잇어야함
class Car{
...
public class Ex03_DataType {
	public static void main(String[] args) {
		...
		// 정수 리터럴 >> 표현 숫자값
		// 접미사 L
		long number = 1000000000000L; // 정수 리터럴 값을 long 으로 인지
		...
		// java 문자열 : "가" : String s = "가" , String s2 = "djkflskdf";
		...
		int cint2 = c4; // 암식적 형변환 사실은 컴파일러가 (int)c4;
		
		// Today Point
		// 변수가 가지고 있는 [값을 보지 말고] [변수의 타입을] 보세요
		// 타입의 크기를 보자
		// 큰 타입에는 작은 타입을 값을 넣을 수 있다
...
//		System.out.println(bytevalue); //쓰레기값
		...
		//String (문자열) 타입
		//Stirng 클래스
		//Stirng str = new String("홍길동")
		//POINT : Stirng 일반 값타입 처럼 써도 전혀 문제가 없다 (int, doulbe 처럼)
		...
		// 실수 (부동소수점)
		// float (4byte)
		// dobule (8byte) ** 실수의 기본 리터럴은 double(정밀도가 높다 :소수부를 더 많이 표현)
